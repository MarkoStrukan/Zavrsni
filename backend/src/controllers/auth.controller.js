...
export async function signup(req, res) {
  ...
  try {
    ...
    const indx = Math.floor(Math.random() * 1000);
    const randomAvatar = `https://api.dicebear.com/9.x/avataaars/svg?seed=${indx}`;
...
try {
  ...
  if (!newUser._id) {
    throw new Error("newUser._id is missing!");
  }
...
export async function login(req, res) {
  ...
  try {
    ...
    if(!isPassCorrect) return res.status(401).json({ message: "Invalid email or password." });
...
export async function onboard(req, res) {
  ...
  try {
    ...
    console.log(`Stream user upserted successfully after onboarding ${updatedUser.fullName}`);
